...
const NewReplyBox = ({ id, replyingTo, onClose }) => {
  ...
  return (
    <form onSubmit={handleSubmit} className="grid md:grid-cols-[auto_1fr_auto] grid-cols-2 bg-white p-6 rounded-lg gap-5">
      ...
      <textarea ref={textarea} value={value} onChange={handleChange} required placeholder="Add a reply..." className="md:col-auto md:row-auto row-start-1 col-start-1 col-end-3 overflow-hidden border rounded-lg resize-none flex-grow h-24 px-4 py-2 outline-none focus:ring-1 focus:ring-purple placeholder:text-gray-500"></textarea>
      <button
